...
def msvis(MS, col='DATA', field='0', xaxis='Chan', yaxis='Amp', fname='fastplot.png', logy=False, markersize=1, extent=0.1):
    ...
    try:
        table = tb.table(MS)
        dat = table.query(query = 'FIELD_ID == {0}'.format(field), columns='{0},TIME'.format(col))
        data = dat.getcol(col)
        if data is None:
        ...
        print "Column '{0}' may not exist. Use 'CPARAM' for calibration tables and 'DATA' for MSs. Columns are {2}.".format(col,field,table.colnames())
        print e
        sys.exit()
